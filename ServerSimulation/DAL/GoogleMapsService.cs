...
namespace ServerSimulation.DAL
{
    ...
    public class GoogleMapsService : IGoogleMapsService
    {
        ...
        //         if (people == null)
        //         {
        //             _logger.LogError("People list is null in CalculateShelterDistancesAsync");
        //             throw new ArgumentNullException(nameof(people));
        //         }

        //         if (shelters == null)
        //         {
        //             _logger.LogError("Shelters list is null in CalculateShelterDistancesAsync");
        //             throw new ArgumentNullException(nameof(shelters));
        //         }

        //         _logger.LogInformation($"CalculateShelterDistancesAsync called with {people.Count} people and {shelters.Count} shelters");
        ...
        /// <summary>
        /// Calculate shelter distances using air distance (Haversine formula)
        /// This is much faster but less accurate than walking distances
        /// </summary>
        public async Task<Dictionary<string, Dictionary<string, double>>> CalculateShelterDistancesAsync(
            List<PersonDto> people,
            List<ShelterDto> shelters)
        {
            ...
            {
                // Add null checks
                if (people == null || shelters == null)
                {
                    throw new ArgumentNullException("People and shelters lists cannot be null");
                }

                _logger.LogInformation($"Calculating air distances for {people.Count} people and {shelters.Count} shelters");

                // Calculate air distances (no API calls needed)
                foreach (var person in people)
                {
                    result[person.Id.ToString()] = new Dictionary<string, double>();
                    ...
                    foreach (var shelter in shelters)
                    {
                        ...
                        result[person.Id.ToString()][shelter.Id.ToString()] = distance;
                    }
                }

                _logger.LogInformation($"Air distance calculation complete for {people.Count * shelters.Count} pairs");

                // Return completed task since this doesn't need to be async anymore
        ...
        /// <summary>
        /// Get cached distance from database
        /// Returns a tuple with distance in meters and duration in seconds
        /// </summary>
        private (int? distanceMeters, int? durationSeconds) GetCachedDistanceSimple(double originLat, double originLng, double destLat, double destLng)
        {
            SqlConnection con = null;
            try
            {
            ...
            {"@ToleranceMeters", 50}
                ...
                using (var cmd = CreateCommandWithStoredProcedure("FC_SP_GetCachedDistance", con, parameters))
                {
                    using (SqlDataReader dr = cmd.ExecuteReader())
                    {
                        if (dr.Read())
                        {
                            int distanceMeters = Convert.ToInt32(dr["distance_meters"]);
                            int durationSeconds = Convert.ToInt32(dr["duration_seconds"]);
